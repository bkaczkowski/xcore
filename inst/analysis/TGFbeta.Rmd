...
molecular_signatures[["motevo_randomized"]] <- 
  molecular_signatures[["motevo"]][sample(nrow(molecular_signatures[["motevo"]])), ]
rownames(molecular_signatures[["motevo_randomized"]]) <- rownames(molecular_signatures[["motevo"]])
molecular_signatures[["jaspar_randomized"]] <- 
  molecular_signatures[["jaspar"]][sample(nrow(molecular_signatures[["jaspar"]])), ]
rownames(molecular_signatures[["jaspar_randomized"]]) <- rownames(molecular_signatures[["jaspar"]])
molecular_signatures[["swissregulon_randomized"]] <- 
  molecular_signatures[["swissregulon"]][sample(nrow(molecular_signatures[["swissregulon"]])), ]
rownames(molecular_signatures[["swissregulon_randomized"]]) <- 
  rownames(molecular_signatures[["swissregulon"]])
```
...
mat <- signatures_mat_24_A549[topA_549, ]
rownames(mat) <- sub(
  pattern = "(.*)\\.(.*)\\.(.*)",
  replacement = "\\2 \\3 \\1",
  x = rownames(mat))
...
mat <- signatures_mat_24_MDA_MB_231[topMDA_MB_231, ]
rownames(mat) <- sub(
  pattern = "(.*)\\.(.*)\\.(.*)",
  replacement = "\\2 \\3 \\1",
  x = rownames(mat))
...
top10A_549 <- top_regression$A_549$DPI$replicate_avg$remap %>%
...
top10MDA_MB_231 <- top_regression$MDA_MB_231$DPI$replicate_avg$remap %>%
  apply(MARGIN = 1, FUN = function(zscore) 2 * (1 - pnorm(abs(zscore)))) %>%
  apply(MARGIN = 2, FUN = fisherMethod) %>%
  sort() %>%
  head(10) %>%
...
ap <- applyOverDFList(
    list_of_df = top_regression$A_549$DPI$pvalues$remap, 
    col_name = "pval", 
    fun = function(x )fisherMethod(x, log.p = FALSE), 
...
mm <- applyOverDFList(
    list_of_df = top_regression$MDA_MB_231$DPI$pvalues$remap, 
    col_name = "coef", 
    fun = mean, 
...
top10A_549 <- top_regression$A_549$DPI$replicate_avg$chip_atlas %>%
  apply(MARGIN = 1, FUN = function(zscore) 2 * (1 - pnorm(abs(zscore)))) %>%
  apply(MARGIN = 2, FUN = fisherMethod) %>%
  sort() %>%
  head(10) %>%
...
top10MDA_MB_231 <- top_regression$MDA_MB_231$DPI$replicate_avg$chip_atlas %>%
  apply(MARGIN = 1, FUN = function(zscore) 2 * (1 - pnorm(abs(zscore)))) %>%
  apply(MARGIN = 2, FUN = fisherMethod) %>%
  sort() %>%
  head(10) %>%
...
am <- applyOverDFList(
    list_of_df = top_regression$A_549$DPI$pvalues$chip_atlas, 
    col_name = "coef", 
    fun = mean, 
    groups = design2factor(designA)) %>%
  subsetWithMissing(rows = rows)
...
mm <- applyOverDFList(
    list_of_df = top_regression$MDA_MB_231$DPI$pvalues$chip_atlas, 
    col_name = "coef", 
    fun = mean, 
    groups = design2factor(designM)) %>%
  subsetWithMissing(rows = rows)
...
y <- top_regression$A_549$DPI$replicate_avg$remap[, "A_TGFb_24h"] %>% abs()
...
y <- top_regression$A_549$DPI$replicate_avg$chip_atlas[, "A_TGFb_24h"] %>% abs()
...
y <- top_regression$MDA_MB_231$DPI$replicate_avg$remap[, "M_TGFb_24h"] %>% abs()
...
y <- top_regression$MDA_MB_231$DPI$replicate_avg$chip_atlas[, "M_TGFb_24h"] %>% abs()
