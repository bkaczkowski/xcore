...
# Microarray TGFbeta dataset Keshamouni et al.

We complement the analysis of CAGE TGFβ treatment data using publicly available
microarray data from TGFβ treatment in A-549 cell line (GSE17708). This dataset
is already normalized and in logarithmic base thus no further normalization is
applied.

```{r}
in_counts <- system.file("inst", "extdata", "GSE17708_Keshamouni_TGFB1_logs.xls", package = "xcore")
countsK <- readxl::read_excel(in_counts, col_names = TRUE, skip = 3)
colnames(countsK) <- countsK[1, ]
countsK <- countsK[-1, ]
countsK <- countsK[, c(1:5, grep("^V[0-9]", colnames(countsK)))]
samplesK <- c(
  "0hr_1", "0hr_2", "0hr_3", # 1
  "0.5hr_1", "0.5hr_2", "0.5hr_3", # 2
  "1hr_1", "1hr_2", "1hr_3", # 3
  "2hr_1", "2hr_2", # 4
  "4hr_1", "4hr_2", "4hr_3", # 5
  "8hr_1", "8hr_2", "8hr_3", # 6
  "16hr_1", "16hr_2", "16hr_3", # 7
  "24hr_1", "24hr_2", "24hr_3", # 8
  "72hr_1", "72hr_2", "72hr_3" # 9
)
colnames(countsK) <- c(
  colnames(countsK)[1:5],
  samplesK
)
```
...
designK <- diag(ncond)[rep(seq_len(ncond), times = table(cond)[unique(cond)]), ]
rownames(designK) <- samplesK
colnames(designK) <- unique(cond)
```

```{r}
countsK_entrez <- countsK[, -c(1:5)] %>% as.matrix() %>% type.convert()
rownames(countsK_entrez) <- countsK$`Entrez Gene`
colnames(countsK_entrez) <- samplesK
countsK_entrez <- countsK_entrez[rownames(countsK_entrez) %in% promoters_f5_core$ENTREZID, ]
entrez2fantom <- setNames(promoters_f5_core$name, promoters_f5_core$ENTREZID)
rownames(countsK_entrez) <- entrez2fantom[rownames(countsK_entrez)]
```

```{r}
designK_wo_base <- designK
designK_wo_base[, "0hr"] <- 0
dataK_symbol <- MultiAssayExperiment::MultiAssayExperiment(
  experiments = MultiAssayExperiment::ExperimentList(
      U = matrix(data = rowMeans(countsK_entrez[, 1:3]), ncol = 1, dimnames = list(rownames(countsK_entrez), "u")),
      Y = countsK_entrez[, samplesK[-c(1:3)]]),
    metadata = list(designK = designK_wo_base)
)
```
...
# Keshamouni A-549
## SYMBOL
# here we only look at ReMap and ChIP-Atlas
dataK_symbol <- addSignatures(dataK_symbol,
                              remap = remap_promoters_f5,
                              chip_atlas = chip_atlas_promoters_f5)
dataK_symbol <- filterSignatures(dataK_symbol)
...
plotScatterPlotWithErrorBars(rsqM, ord_remap, xsize_remap, title_suffix = "MDA-231-D ReMap2020 24h")
plotBoxplotWithTwoGroups(rsqM, ord_remap, title_suffix = "MDA-231-D ReMap2020 24h")
```
...
```{r eval=FALSE}
top_regression <- list(A_549 = list(), MDA_MB_231 = list(), Keshamouni = list())
...
top_regression[["Keshamouni"]][["SYMBOL"]] <- modelGeneExpression(
  mae = dataK_symbol,
  yname = "Y",
  uname = "U",
  xnames = xnames,
...
plotTopSignaturesHeatmap <- function(signature = "remap") {
  ...
  top10Keshamouni <- apply(top_regression$Keshamouni$SYMBOL$zscore_avg[[signature]], 1, stoufferZMethod) %>%
    abs() %>%
    sort(decreasing = TRUE) %>%
    head(10) %>%
    names()

  rows <- Reduce(base::union, list(top10A_549, top10MDA_MB_231, top10Keshamouni))
  ...
  km <- applyOverDFList(
      list_of_df = top_regression$Keshamouni$SYMBOL$pvalues[[signature]],
      col_name = "coef",
      fun = mean,
      groups = design2factor(metadata(dataK_symbol)$design)) %>%
    subsetWithMissing(rows = rows)
  km <- km[, c("1hr", "2hr", "4hr", "8hr", "16hr", "24hr")]

  mats_cb <- do.call(cbind, list(am, mm, km))
  i_na <- is.na(mats_cb)
  mats_cb[i_na] <- 0
  ...
  graphics::layout(
    mat = matrix(
      ...
      byrow = TRUE),
    heights = c(1, 4),
    widths = c(1, 3, 3, 1, 4, 8)
  )

  breaks <- seq(from = -0.07, to = 0.07, length.out = 10) # even to map white to 0
  ...
  # Keshamouni
  breaks <- seq(from = -0.015, to = 0.015, length.out = 10) # even to map white to 0

  plotHeatmapScaleBar(breaks, main = "activity", mar = c(1.1, 4.1, 3.1, 2.1))

  mat <- km[i, ]
  colnames(mat) <- sub("hr", "h", colnames(mat))
  plotHeatmap(
    mat,
    breaks,
    main = "A-549 (GSE17708)",
