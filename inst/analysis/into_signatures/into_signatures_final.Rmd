...
yule <- proxy::dist(x = mat, 
                    method = "Yule", 
                    by_rows = FALSE, 
                    convert_similarities = FALSE) %>% proxy::as.matrix(diag = 1)
d_yule <- sqrt(1 - yule)

yule2 <- proxy::dist(x = mat, 
                     method = "Yule2", 
                     by_rows = FALSE, 
                     convert_similarities = FALSE) %>% proxy::as.matrix(diag = 1)
d_yule2 <- sqrt(1- yule2)
...
d_pearson <- sqrt(1 - cor(mat, method = "pearson")) %>% as.dist()
...
plotExperimentsHeatmap(
  ...
  clustering_distance_rows = d_pearson,
  clustering_distance_cols = d_pearson,
  clustering_method = "ward.D2",
  breaks = breaks,
  color = color,
...
# TODO redo the clustering on all promoters?

```{r}
load(system.file("inst", "analysis", "into_signatures", "d_yule_remap.rda", package = "xcore"))
load(system.file("inst", "analysis", "into_signatures", "d_yule_chip_atlas.rda", package = "xcore"))
```
...
remap_promoters_core_collapsed_3 <- 
...
save(remap_promoters_core_collapsed_3, file = "remap_promoters_core_collapsed_3.rda")

remap_promoters_collapsed_3 <- 
  collapseInteractionMatrix(mat = remap_promoters[, ids_remap], 
                            ...
                            purity_feature = "tf", 
                            min_pruning_purity = 0.5) # clusters with purity > 0.5 are pruned
save(remap_promoters_collapsed_3, file = "remap_promoters_collapsed_3.rda")

meta_remap[, cluster := cl_remap]
data.table::fwrite(meta_remap, file = "remap_metadata_clusters_3.csv")
####
...
res[res[, (AMI * ARI) == max(AMI * ARI)]]
#    deepSplit cutHeight       ARI       AMI   N
# 1:         1       2.5 0.4428525 0.7467066 463
...
remap_promoters_core_collapsed_1 <- 
  collapseInteractionMatrix(mat = remap_promoters_core[, ids_remap], 
                            meta = meta_remap, 
                            cl = cl_remap, 
                            alpha = 0.5, # 0.5 corresponds to majority vote
                            purity_feature = "tf", 
                            min_pruning_purity = 0.5) # clusters with purity > 0.5 are pruned
save(remap_promoters_core_collapsed_1, file = "remap_promoters_core_collapsed_1.rda")

remap_promoters_collapsed_1 <- 
  collapseInteractionMatrix(mat = remap_promoters[, ids_remap], 
                            meta = meta_remap, 
                            cl = cl_remap, 
                            alpha = 0.5, # 0.5 corresponds to majority vote
                            purity_feature = "tf", 
                            min_pruning_purity = 0.5) # clusters with purity > 0.5 are pruned
save(remap_promoters_collapsed_1, file = "remap_promoters_collapsed_1.rda")

meta_remap[, cluster := cl_remap]
data.table::fwrite(meta_remap, file = "remap_metadata_clusters_1.csv")
####
...
remap_promoters_core_collapsed_0.5_a0.5 <- 
  collapseInteractionMatrix(mat = remap_promoters_core[, ids_remap], 
                            meta = meta_remap, 
                            cl = cl_remap, 
                            alpha = 0.5, # 0.5 corresponds to majority vote
                            purity_feature = "tf", 
                            min_pruning_purity = 0.5) # clusters with purity > 0.5 are pruned
save(remap_promoters_core_collapsed_0.5_a0.5, file = "remap_promoters_core_collapsed_0.5_a0.5.rda")
...
meta_remap[, cluster := cl_remap]
data.table::fwrite(meta_remap, file = "remap_metadata_clusters_0.5_a0.5.csv")
####
...
chip_atlas_promoters_core_collapsed_3 <- 
  collapseInteractionMatrix(mat = chip_atlas_promoters_core[, ids_chip_atlas], 
                            ...
                            purity_feature = "tf", 
                            min_pruning_purity = 0.5) # clusters with purity > 0.5 are pruned
save(chip_atlas_promoters_core_collapsed_3, file = "chip_atlas_promoters_core_collapsed_3.rda")

chip_atlas_promoters_collapsed_3 <- 
  collapseInteractionMatrix(mat = chip_atlas_promoters[, ids_chip_atlas], 
                            ...
                            purity_feature = "tf", 
                            min_pruning_purity = 0.5) # clusters with purity > 0.5 are pruned
save(chip_atlas_promoters_collapsed_3, file = "chip_atlas_promoters_collapsed_3.rda")

meta_chip_atlas[, cluster := cl_chip_atlas]
data.table::fwrite(meta_chip_atlas, file = "chip_atlas_metadata_clusters_3.csv")
####
...
chip_atlas_promoters_core_collapsed_1 <- 
  collapseInteractionMatrix(mat = chip_atlas_promoters_core[, ids_chip_atlas], 
                            meta = meta_chip_atlas, 
                            cl = cl_chip_atlas, 
                            alpha = 0.5, # 0.5 corresponds to majority vote
                            purity_feature = "tf", 
                            min_pruning_purity = 0.5) # clusters with purity > 0.5 are pruned
save(chip_atlas_promoters_core_collapsed_1, file = "chip_atlas_promoters_core_collapsed_1.rda")

chip_atlas_promoters_collapsed_1 <- 
  collapseInteractionMatrix(mat = chip_atlas_promoters[, ids_chip_atlas], 
                            meta = meta_chip_atlas, 
                            cl = cl_chip_atlas, 
                            alpha = 0.5, # 0.5 corresponds to majority vote
                            purity_feature = "tf", 
                            min_pruning_purity = 0.5) # clusters with purity > 0.5 are pruned
save(chip_atlas_promoters_collapsed_1, file = "chip_atlas_promoters_collapsed_1.rda")

meta_chip_atlas[, cluster := cl_chip_atlas]
data.table::fwrite(meta_chip_atlas, file = "chip_atlas_metadata_clusters_1.csv")
####
```
...
```{r}
cut_height = seq(0.05, 1.5, by = 0.4)
alpha <- 0.5 # majority vote
static <- list(remap = list(), chip_atlas = list())
for (db_name in c("remap", "chip_atlas")) {
  db <- get(paste0(db_name, "_promoters_core"))
  j <- Matrix::colSums(db) >= (length(promoters_ann_core) * 0.01)
  db <- db[, j]
  db_meta <- get(paste0(db_name, "_meta"))
  db_meta <- db_meta[id %in% colnames(db), ]
  tfs <- db_meta[, unique(tf)]
  for (cheight in cut_height) {
    for (tf_name in tfs) {
      sigid <- db_meta[tf == tf_name, id]
      if (length(sigid) == 1) {
        cl_static <- stats::setNames(
          paste0(tf_name, "_cl1_1_R1"),
          sigid
        )
      } else {
        d_yule_sig <- proxy::dist(
          x = as.matrix(db[, sigid]),
          method = "Yule",
          by_rows = FALSE,
          convert_similarities = TRUE)
        hc_sig <- hclust(d_yule_sig, method = "ward.D2")
        cl_static <- cutree(hc_sig, h = cheight)
        vec_names <- names(cl_static)
        cl_static <-
          paste0(tf_name, "_cl", cl_static, "_", max(cl_static))
        reps <- table(cl_static)
        cl_static <-
          paste0(cl_static, "_R", reps[cl_static])
        names(cl_static) <- vec_names
      }
      static[[db_name]][[as.character(cheight)]] <-
        c(static[[db_name]][[as.character(cheight)]], cl_static)
    }
    cl <- static[[db_name]][[as.character(cheight)]][colnames(db)]
    cl <- factor(cl)
    inmat <- get(paste0(db_name, "_promoters_core"))
    intmat <- do.call(what = cbind, args = lapply(
      X = levels(cl), function(clu) 
        simplifyInteractionMatrix(mat = inmat[, names(cl)[cl == clu], drop = FALSE],
                                  alpha = alpha,
                                  colname = clu)))
    assign(
      x = paste0(db_name, "_per_tf_core_collapsed_", cheight),
      value = intmat
    )
    save(list = paste0(db_name, "_per_tf_core_collapsed_", cheight),
         file = paste0(db_name, "_per_tf_core_collapsed_", cheight, ".rda"))
  }
}
```
...
for (db_name in c("remap", "chip_atlas")) {
  ...
  for (cheight in cut_height) {
    ...
    # core
    inmat <- get(paste0(db_name, "_promoters_core"))
    intvecs <- lapply(X = levels(cl), function(clu)
      Matrix::rowSums(inmat[, cl == clu, drop = FALSE], sparseResult = TRUE))
    ...
    intmat <- Matrix::sparseMatrix(i = i, j = j, x = x,
                                   dimnames = list(rownames(inmat), levels(cl)))
    assign(
      x = paste0(db_name, "_per_tf_core_collapsed_rowsums", cheight),
      value = intmat
    )
    save(list = paste0(db_name, "_per_tf_core_collapsed_rowsums", cheight),
         file = paste0(db_name, "_per_tf_core_collapsed_rowsums", cheight, ".rda"))
    
    # all
    inmat <- get(paste0(db_name, "_promoters"))
    intvecs <- lapply(X = levels(cl), function(clu)
      Matrix::rowSums(inmat[, names(cl)[cl == clu], drop = FALSE], sparseResult = TRUE))
    i <- do.call(c, lapply(intvecs, slot, "i"))
    j_lens <- vapply(intvecs, function(v) length(slot(v, "i")), numeric(1L))
    j <- do.call(c, lapply(seq_along(j_lens), function(l) rep(1L, times = j_lens[l]) * l))
    x <- do.call(c, lapply(intvecs, slot, "x"))
    intmat <- Matrix::sparseMatrix(i = i, j = j, x = x,
                                   dimnames = list(rownames(inmat), levels(cl)))
    assign(
      x = paste0(db_name, "_per_tf_collapsed_rowsums", cheight),
      value = intmat
    )
    save(list = paste0(db_name, "_per_tf_collapsed_rowsums", cheight),
         file = paste0(db_name, "_per_tf_collapsed_rowsums", cheight, ".rda"))
  }
}
```

## DPI rowSums -- integer overflow, matrices are too long...
```{r}
cut_height = seq(0.01, 1.5, by = 0.4)
static <- list(remap = list(), chip_atlas = list())
for (db_name in c("remap", "chip_atlas")) {
  db <- get(paste0(db_name, "_promoters"))
  db <- db[rownames(db) %in% sel_dpi, ] # readLines("dataA_dpi.txt")
  j <- Matrix::colSums(db) >= (length(promoters_ann_core) * 0.01)
  db <- db[, j]
  db_meta <- get(paste0(db_name, "_meta"))
  db_meta <- db_meta[id %in% colnames(db), ]
  ...
  for (cheight in cut_height) {
    ...
    intmat <- Matrix::sparseMatrix(i = i, j = j, x = x,
                                   dimnames = list(rownames(inmat), levels(cl)))
    assign(
      x = paste0(db_name, "_per_tf_core_collapsed_rowsums_dpi_A", cheight),
      value = intmat
    )
    save(list = paste0(db_name, "_per_tf_core_collapsed_rowsums_dpi_A", cheight),
         file = paste0(db_name, "_per_tf_core_collapsed_rowsums_dpi_A", cheight, ".rda"))
    ...
    intmat <- Matrix::sparseMatrix(i = i, j = j, x = x,
                                   dimnames = list(rownames(inmat), levels(cl)))
    assign(
      x = paste0(db_name, "_per_tf_collapsed_rowsums_dpi_A", cheight),
      value = intmat
    )
    save(list = paste0(db_name, "_per_tf_collapsed_rowsums_dpi_A", cheight),
         file = paste0(db_name, "_per_tf_collapsed_rowsums_dpi_A", cheight, ".rda"))
  }
}
```
...
for (db_name in c("remap", "chip_atlas")) {
  ...
  for (cheight in cut_height) {
    ...
    intmat <- Matrix::sparseMatrix(i = i, j = j, x = x,
                                   dimnames = list(rownames(inmat), levels(cl)))
    assign(
      x = paste0(db_name, "_per_tf_core_collapsed_rowsums_fix", cheight),
      value = intmat
    )
    save(list = paste0(db_name, "_per_tf_core_collapsed_rowsums_fix", cheight),
         file = paste0(db_name, "_per_tf_core_collapsed_rowsums_fix", cheight, ".rda"))
    ...
    intmat <- Matrix::sparseMatrix(i = i, j = j, x = x,
                                   dimnames = list(rownames(inmat), levels(cl)))
    assign(
      x = paste0(db_name, "_per_tf_collapsed_rowsums_fix", cheight),
      value = intmat
    )
    save(list = paste0(db_name, "_per_tf_collapsed_rowsums_fix", cheight),
         file = paste0(db_name, "_per_tf_collapsed_rowsums_fix", cheight, ".rda"))
  }
}
```
  ...
  for (cheight in cut_height) {
    for (tf_name in tfs) {
      ...
      if (length(sigid) == 1) {
        ...
        d_sig <- sqrt(1 - cor(as.matrix(db[, sigid]), method = "pearson")) %>% as.dist()
        hc_sig <- hclust(d_sig, method = "ward.D2")
        cl_static <- cutree(hc_sig, h = cheight)
        vec_names <- names(cl_static)
        cl_static <-
          paste0(tf_name, "_cl", cl_static, "_", max(cl_static))
    ...
    intmat <- Matrix::sparseMatrix(i = i, j = j, x = x,
                                   dimnames = list(rownames(inmat), levels(cl)))
    assign(
      x = paste0(db_name, "_per_tf_core_collapsed_rowsums_pearson", cheight),
      value = intmat
    )
    save(list = paste0(db_name, "_per_tf_core_collapsed_rowsums_pearson", cheight),
         file = paste0(db_name, "_per_tf_core_collapsed_rowsums_pearson", cheight, ".rda"))
    ...
    intmat <- Matrix::sparseMatrix(i = i, j = j, x = x,
                                   dimnames = list(rownames(inmat), levels(cl)))
    assign(
      x = paste0(db_name, "_per_tf_collapsed_rowsums_pearson", cheight),
      value = intmat
    )
    save(list = paste0(db_name, "_per_tf_collapsed_rowsums_pearson", cheight),
         file = paste0(db_name, "_per_tf_collapsed_rowsums_pearson", cheight, ".rda"))
  }
}
```
