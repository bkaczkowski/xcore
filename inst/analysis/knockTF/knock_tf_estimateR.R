...
for (i in seq(argv[1], argv[2])) {
  ...
  load(paste0(dump_isilon, samples[i], ".rda")) # gives res
  cols <- colnames(mae[["Y"]])
  stat <- foreach(i = seq_along(cols), .combine = c) %dopar%
    estimateStat(
      x = mae[["remap"]],
      y = mae[["Y"]][, i],
      u = mae[["U"]],
      s = res$regression_models$remap[[i]]$lambda.min,
      statistic = rsq)
  rsq_list[[samples[i]]] <- stat
}

save(rsq_list, file = paste0("rsq_list_", argv[1], "_", argv[2], ".rda"))
