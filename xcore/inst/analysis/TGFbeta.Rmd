...
# A-549
xnames <- setdiff(names(dataA), c("U", "Y", "DE", "remap_core_raw")
...
# MDA-MB-231
xnames <- setdiff(names(dataM), c("U", "Y", "DE", "remap_core_raw")
designA <- design_mat[grepl("M_", rownames(design_mat)), grepl("M_", colnames(design_mat))]
...
MSE <- list()
for (nm in xnames) {
  for(mod in regression[["A_549"]][["regression_models"]][[nm]][j]) {
    stat <-
...
MSE <- list()
for (nm in xnames) {
  for(mod in regression[["MDA_MB_231"]][["regression_models"]][[nm]][j]) {
    stat <-
