...
## motif activity vs ridge regression
```{r}
i <- base::intersect(rownames(A_activities_remap_fc), rownames(A_remap_ridge_coeff))
arcor <- cor(A_activities_remap_fc[i, 5], A_remap_ridge_coeff[i, 5])
plot(
  x = A_activities_remap_fc[i, 5],
  y = A_remap_ridge_coeff[i, 5],
  main = sprintf("Ridge regression vs Motif Activity\nA 24h Pearson: %.3f", arcor),
  xlab = "motif activity",
  ylab = "ridge regression")

## significant coeff only
j <- rownames(A_remap_ridge_pvalue)[A_remap_ridge_pvalue[, 5] > -log10(0.05)]
i <- base::intersect(i, j)
arcor <- cor(A_activities_remap_fc[i, 5], A_remap_ridge_coeff[i, 5])
plot(
  x = A_activities_remap_fc[i, 5],
  y = A_remap_ridge_coeff[i, 5],
  main = sprintf("Ridge regression vs Motif Activity (significant coeff only)\nA 24h Pearson: %.3f", arcor),
  xlab = "motif activity",
  ylab = "ridge regression")
...
## fisher results vs ridge results

up/down fisher results could be combined always choose bigger odds ratio and add
sign, then you can compare odds ratio to coeffs. Take log2 odds ratio.
Use edgeR "DE" as an input.
```{r}
i <- base::intersect(rownames(fisher_remap$A$odds), rownames(A_remap_ridge_coeff))
frcor <- cor(fisher_remap$A$odds[i, 5], A_remap_ridge_coeff[i, 5])
plot(
  x = fisher_remap$A$odds[i, 5],
  y = A_remap_ridge_coeff[i, 5],
  main = sprintf("Ridge regression vs Fisher odds\nA 24h Pearson: %.3f", frcor),
  xlab = "Fisher's odds",
  ylab = "ridge regression")

## significant coeff only
j <- rownames(A_remap_ridge_pvalue)[A_remap_ridge_pvalue[, 5] > -log10(0.05)]
frcor <- cor(fisher_remap$A$odds[j, 5], A_remap_ridge_coeff[j, 5])
plot(
  x = fisher_remap$A$odds[j, 5],
  y = A_remap_ridge_coeff[j, 5],
  main = sprintf("Ridge regression vs Fisher odds (significant coeff only)\nA 24h Pearson: %.3f", frcor),
  xlab = "Fisher's odds",
  ylab = "ridge regression")

## significant odds only
j <- rownames(fisher_remap$A$p.value)[fisher_remap$A$p.value[, 5] < 0.05]
frcor <- cor(fisher_remap$A$odds[j, 5], A_remap_ridge_coeff[j, 5])
plot(
  x = fisher_remap$A$odds[j, 5],
  y = A_remap_ridge_coeff[j, 5],
  main = sprintf("Fisher odds vs Ridge regression (significant odds only)\nA 24h Pearson: %.3f", frcor),
  xlab = "Fisher's odds",
  ylab = "ridge regression")
```

```{r}
i <- base::intersect(rownames(fisher_remap$A$odds), rownames(A_activities_remap_fc))
facor <- cor(fisher_remap$A$odds[i, 5], A_activities_remap_fc[i, 5])
plot(
  x = fisher_remap$A$odds[i, 5],
  y = A_activities_remap_fc[i, 5],
  main = sprintf("Fisher odds vs Motif Activity\nA 24h Pearson: %.3f", facor),
  xlab = "Fisher's odds",
  ylab = "motif activity")

## significant odds only
j <- rownames(fisher_remap$A$p.value)[fisher_remap$A$p.value[, 5] < 0.05]
facor <- cor(fisher_remap$A$odds[j, 5], A_activities_remap_fc[j, 5])
plot(
  x = fisher_remap$A$odds[j, 5],
  y = A_activities_remap_fc[j, 5],
  main = sprintf("Fisher odds vs motif activity (significant odds only)\nA 24h Pearson: %.3f", facor),
  xlab = "Fisher's odds",
  ylab = "motif activity")
```
