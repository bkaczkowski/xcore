...
# TODO DPI level analysis
# counts_symbol <- counts_dpi
```
...
load(
  system.file(
    ...
    "analysis",
    "into_signatures",
    "remap_promoters_core_collapsed.rda",
    package = "xcore"
  )
)
...
load(
  system.file(
    ...
    "analysis",
    "into_signatures",
    "chip_atlas_promoters_core_collapsed.rda",
    package = "xcore"
  )
)

# translate collapsed core promoters signatures to symbol level
rownames(remap_promoters_core_collapsed) <-
  dpi2symbol[rownames(remap_promoters_core_collapsed)]
rownames(chip_atlas_promoters_core_collapsed) <-
  dpi2symbol[rownames(chip_atlas_promoters_core_collapsed)]

# change to new cluster labels
remap_clusters_meta_file <-
  system.file("inst",
              "analysis",
              "into_signatures",
              "remap_metadata_clusters.csv",
              package = "xcore")
remap_clusters_meta <- data.table::fread(remap_clusters_meta_file)
new_labels <- remap_clusters_meta[, paste0(cluster, "_", makeClusterName2(.SD)), by = cluster]
new_labels <- stats::setNames(new_labels[["V1"]], new_labels[["cluster"]])
new_labels <- new_labels[order(as.numeric(names(new_labels)))]
colnames(remap_promoters_core_collapsed) <- new_labels

chip_atlas_clusters_meta_file <-
  system.file("inst",
              "analysis",
              "into_signatures",
              "chip_atlas_metadata_clusters.csv",
              package = "xcore")
chip_atlas_clusters_meta <- data.table::fread(chip_atlas_clusters_meta_file)
new_labels <- chip_atlas_clusters_meta[, paste0(cluster, "_", makeClusterName2(.SD)), by = cluster]
new_labels <- stats::setNames(new_labels[["V1"]], new_labels[["cluster"]])
new_labels <- new_labels[order(as.numeric(names(new_labels)))]
colnames(chip_atlas_promoters_core_collapsed) <- new_labels
...
motevo_count <- getInteractionMatrix(promoters_ann_core, motevo, count = TRUE)
rownames(motevo_count) <- dpi2symbol[rownames(motevo_count)]
motevo_binary <- as(as(motevo_count, "lgCMatrix"), "dgCMatrix")
rm(motevo)
...
jaspar_count <- getInteractionMatrix(promoters_ann_core, jaspar, count = TRUE)
rownames(jaspar_count) <- dpi2symbol[rownames(jaspar_count)]
jaspar_binary <- as(as(jaspar_count, "lgCMatrix"), "dgCMatrix")
rm(jaspar)
```
...
remap_core_collapsed_rand1 <- 
  remap_promoters_core_collapsed[sample(nrow(remap_promoters_core_collapsed)), ]
rownames(remap_core_collapsed_rand1) <- rownames(remap_promoters_core_collapsed)
remap_core_collapsed_rand2 <- 
  remap_promoters_core_collapsed[sample(nrow(remap_promoters_core_collapsed)), ]
rownames(remap_core_collapsed_rand2) <- rownames(remap_promoters_core_collapsed)

# A-549
dataA <- addSignatures(
  dataA,
  remap_core_collapsed = remap_promoters_core_collapsed,
  remap_core_collapsed_rand = remap_core_collapsed_rand1,
  remap_core_raw = remap_raw_core,
  remap_A549 = remap_raw_core[, grep("A-549", colnames(remap_raw_core))],
  remap_MCF7 = remap_raw_core[, grep("MCF-7", colnames(remap_raw_core))],
  chip_atlas_core_collapsed = chip_atlas_promoters_core_collapsed,
  chip_atlas_A549 = chip_atlas_raw_core[, grep("A549", colnames(chip_atlas_raw_core))],
  chip_atlas_MCF7 = chip_atlas_raw_core[, grep("MCF-7", colnames(chip_atlas_raw_core))],
  motevo_count = motevo_count,
  motevo_binary = motevo_binary,
  jaspar_count = jaspar_count,
  jaspar_binary = jaspar_binary,
...
dataM <- addSignatures(
  dataM,
  remap_core_collapsed = remap_promoters_core_collapsed,
  remap_core_collapsed_rand = remap_core_collapsed_rand2,
  remap_core_raw = remap_raw_core,
  remap_A549 = remap_raw_core[, grep("A-549", colnames(remap_raw_core))],
  remap_MCF7 = remap_raw_core[, grep("MCF-7", colnames(remap_raw_core))],
  chip_atlas_core_collapsed = chip_atlas_promoters_core_collapsed,
...
## Ridge regression
```{r eval=FALSE}
regression_models <- list(A_549 = list(), MDA_MB_231 = list())

# A-549
Xnms <- setdiff(names(dataA), c("U", "Y", "DE", "remap_core_raw"))
for (xnm in Xnms) {
  regression_models[["A_549"]][[xnm]] <-
    foreach(y = iterators::iter(dataA[["Y"]])) %dopar%
      runLinearRidge(x = dataA[[xnm]], 
                    y = y[, 1, drop = TRUE],
                    offset = dataA[["U"]])
  names(regression_models[["A_549"]][[xnm]]) <- colnames(dataA[["Y"]])
}

# MDA-MB-231
Xnms <- setdiff(names(dataM), c("U", "Y", "DE", "remap_core_raw"))
for (xnm in Xnms) {
  regression_models[["MDA_MB_231"]][[xnm]] <-
    foreach(y = iterators::iter(dataM[["Y"]])) %dopar%
      runLinearRidge(x = dataM[[xnm]], 
                    y = y[, 1, drop = TRUE],
                    offset = dataM[["U"]])
  names(regression_models[["MDA_MB_231"]][[xnm]]) <- colnames(dataM[["Y"]])
}

# simple replicates handling
regression_simple_avg <- list(A_549 = list(), MDA_MB_231 = list())
...
groups <- dataA[["Y"]] %>% 
  ...
  unique()
for (xnm in Xnms) {
  regression_simple_avg[["A_549"]][[xnm]] <- 
    foreach(gr = groups) %dopar%
      {
        i <- grep(gr, names(regression_models[["A_549"]][[xnm]]))
        getAvgCoeff(regression_models[["A_549"]][[xnm]][i])
      }
  regression_simple_avg[["A_549"]][[xnm]] <- 
    do.call(what = cbind, 
            args = lapply(regression_simple_avg[["A_549"]][[xnm]], function (x) x[, "z"]))
  colnames(regression_simple_avg[["A_549"]][[xnm]]) <- groups
}
...
groups <- dataM[["Y"]] %>% 
  ...
  unique()
for (xnm in Xnms) {
  regression_simple_avg[["MDA_MB_231"]][[xnm]] <- 
    foreach(gr = groups) %dopar%
      {
        i <- grep(gr, names(regression_models[["MDA_MB_231"]][[xnm]]))
        getAvgCoeff(regression_models[["MDA_MB_231"]][[xnm]][i])
      }
  regression_simple_avg[["MDA_MB_231"]][[xnm]] <- 
    do.call(what = cbind, 
            args = lapply(regression_simple_avg[["MDA_MB_231"]][[xnm]], function (x) x[, "z"]))
  colnames(regression_simple_avg[["MDA_MB_231"]][[xnm]]) <- groups
}

# elaborate replicates handling
regression_elaborate_pvalues <- list(A_549 = list(), MDA_MB_231 = list())
regression_elaborate_avg <- list(A_549 = list(), MDA_MB_231 = list())
...
for (xnm in Xnms) {
  ...
  X <- scale(dataA[[xnm]])
  svdX <- svd(X)
  regression_elaborate_pvalues[["A_549"]][[xnm]] <-
    foreach(id = colnames(dataA[["Y"]])) %dopar%
      {
        y <- dataA[["Y"]][, id] - dataA[["U"]]
        lambda <- regression_models[["A_549"]][[xnm]][[id]]$lambda.min
        beta <- coef(regression_models[["A_549"]][[xnm]][[id]], s = lambda)
  ...
  names(regression_elaborate_pvalues[["A_549"]][[xnm]]) <- colnames(dataA[["Y"]])
  
  # calculate averaged z-scores
  regression_elaborate_avg[["A_549"]][[xnm]] <- foreach(gr = groups) %dopar%
    {
      i <- grep(gr, names(regression_elaborate_pvalues[["A_549"]][[xnm]]))
      se <- lapply(regression_elaborate_pvalues[["A_549"]][[xnm]][i], function (x) x$se)
      se <- do.call(cbind, se)
      se <- apply(se, 1, function (x) sqrt(sum(x * x) / length(x))) # Cohen, J. (1988)
      estimate <- lapply(regression_elaborate_pvalues[["A_549"]][[xnm]][i], function (x) x$coef)
      estimate <- do.call(cbind, estimate)
      estimate <- apply(estimate, 1, function (x) mean(x))
      estimate / se
    }
  regression_elaborate_avg[["A_549"]][[xnm]] <- 
    do.call(cbind, regression_elaborate_avg[["A_549"]][[xnm]])
  rownames(regression_elaborate_avg[["A_549"]][[xnm]]) <- colnames(X)
  colnames(regression_elaborate_avg[["A_549"]][[xnm]]) <- groups
}
...
for (xnm in Xnms) {
  ...
  X <- scale(dataM[[xnm]])
  svdX <- svd(X)
  regression_elaborate_pvalues[["MDA_MB_231"]][[xnm]] <-
    foreach(id = colnames(dataM[["Y"]])) %dopar%
      {
        y <- dataM[["Y"]][, id] - dataM[["U"]]
        lambda <- regression_models[["MDA_MB_231"]][[xnm]][[id]]$lambda.min
        beta <- coef(regression_models[["MDA_MB_231"]][[xnm]][[id]], s = lambda)
        beta <- beta[-1, ] # drop intercept
        pval_test <- ridgePValues(
          x = X,
          y = y, 
  ...
  names(regression_elaborate_pvalues[["MDA_MB_231"]][[xnm]]) <- colnames(dataM[["Y"]])
  
  # calculate averaged z-scores
  regression_elaborate_avg[["MDA_MB_231"]][[xnm]] <- foreach(gr = groups) %dopar%
    {
      i <- grep(gr, names(regression_elaborate_pvalues[["MDA_MB_231"]][[xnm]]))
      se <- lapply(regression_elaborate_pvalues[["MDA_MB_231"]][[xnm]][i], function (x) x$se)
      se <- do.call(cbind, se)
      se <- apply(se, 1, function (x) sqrt(sum(x * x) / length(x))) # Cohen, J. (1988)
      estimate <- lapply(regression_elaborate_pvalues[["MDA_MB_231"]][[xnm]][i], function (x) x$coef)
      estimate <- do.call(cbind, estimate)
      estimate <- apply(estimate, 1, function (x) mean(x))
      estimate / se
    }
  regression_elaborate_avg[["MDA_MB_231"]][[xnm]] <- 
    do.call(cbind, regression_elaborate_avg[["MDA_MB_231"]][[xnm]])
  rownames(regression_elaborate_avg[["MDA_MB_231"]][[xnm]]) <- colnames(X)
  colnames(regression_elaborate_avg[["MDA_MB_231"]][[xnm]]) <- groups
}

save(regression_models, file = "regression_models.rda")
save(regression_simple_avg, file = "regression_simple_avg.rda")
save(regression_elaborate_pvalues, file = "regression_elaborate_pvalues.rda")
save(regression_elaborate_avg, file = "regression_elaborate_avg.rda")
```

# load regression ressults
```{r}
load("regression_models.rda")
load("regression_simple_avg.rda")
load("regression_elaborate_pvalues.rda")
load("regression_elaborate_avg.rda")
```
...
rows <- intersect(
  ...
  y = rownames(A_activities_remap_collapsed_fc)) %>%
    intersect(y = rownames(regression_elaborate_avg[["A_549"]][["remap_core_collapsed"]]))
df <- data.frame(
  ...
  regression_simple = regression_simple_avg[["A_549"]][["remap_core_collapsed"]][rows, 6],
  regression_elaborate = regression_elaborate_avg[["A_549"]][["remap_core_collapsed"]][rows, 6])
...
rows <- intersect(
  ...
  y = rownames(M_activities_remap_collapsed_fc)) %>%
    intersect(y = rownames(regression_elaborate_avg[["MDA_MB_231"]][["remap_core_collapsed"]]))
df <- data.frame(
  ...
  regression_simple = regression_simple_avg[["MDA_MB_231"]][["remap_core_collapsed"]][rows, 6],
  regression_elaborate = regression_elaborate_avg[["MDA_MB_231"]][["remap_core_collapsed"]][rows, 6])
...
compareHeatmaps <- function(rows, ...) {
  ...
  hridge <- regression_elaborate_avg[["A_549"]][["remap_core_collapsed"]] %>%
    `[`(rows, 1:6) %>%
    apply(MARGIN = 1, FUN = scale, center = FALSE) %>%
    t() %>%
...
xcore::select_top_var(regression_elaborate_avg[["A_549"]][["remap_core_collapsed"]]) %>%
  rownames() %>%
  compareHeatmaps(main_heatmap = "regression", column_title = "top variable, A-549")
xcore::select_top_mean_up(regression_elaborate_avg[["A_549"]][["remap_core_collapsed"]]) %>%
  rownames() %>%
  compareHeatmaps(main_heatmap = "regression", column_title = "top mean up, A-549")
xcore::select_top_mean_down(regression_elaborate_avg[["A_549"]][["remap_core_collapsed"]]) %>%
  rownames() %>%
...
compareHeatmaps <- function(rows, ...) {
  ...
  hactiv <- M_activities_remap_collapsed_fc %>%
    ...
    t() %>%
    makeHeatmap(name = "motif activity", column_title = "motif activity")
  hridge <- regression_elaborate_avg[["MDA_MB_231"]][["remap_core_collapsed"]] %>%
    `[`(rows, 1:6) %>%
    apply(MARGIN = 1, FUN = scale, center = FALSE) %>%
    t() %>%
    makeHeatmap(name = "regression", column_title = "regression")
...
# top regression coefficients
xcore::select_top_var(regression_elaborate_avg[["MDA_MB_231"]][["remap_core_collapsed"]]) %>%
  rownames() %>%
  compareHeatmaps(main_heatmap = "regression", column_title = "top variable, MDA-MB-231")
xcore::select_top_mean_up(regression_elaborate_avg[["MDA_MB_231"]][["remap_core_collapsed"]]) %>%
  rownames() %>%
  compareHeatmaps(main_heatmap = "regression", column_title = "top mean up, MDA-MB-231")
xcore::select_top_mean_down(regression_elaborate_avg[["MDA_MB_231"]][["remap_core_collapsed"]]) %>%
  rownames() %>%
...
models_cv <- list()
for (nm in xnames) {
  models_cv[[nm]] <- foreach(y = iterators::iter(dataA[["Y"]][, j])) %dopar%
    glmnet::cv.glmnet(
      x = dataA[[nm]],
      y = y[, 1],
      alpha = 0,
...
MSE <- list()
for (nm in xnames) {
  for(mod in models_cv[[nm]]) {
    stat <-
      foreach(y = iterators::iter(dataA[["Y"]][, j]), .combine = c) %dopar%
        estimateStat(
          x = dataA[[nm]],
          y = y[, 1],
          u = dataA[["U"]],
          s = mod$lambda.min,
          statistic = mse)
    MSE[[nm]] <- c(MSE[[nm]], stat)
  }
}
par(mar = c(14, 4, 4, 2) + 0.1)
mse_bplot <-
  graphics::boxplot(MSE,
        main = "CV MSE across replicates, A-549",
        ylab = "MSE",
        xlab = "",
        notch = TRUE,
        las=2,
        cex.axis = 0.8)
abline(h = median(MSE[["remap_core_collapsed_rand"]]), col = "red", lty = 2)
abline(h = mse_bplot$conf[1, 2], col = "blue", lty = 2)
    ...
    stat <-
      foreach(y = iterators::iter(dataA[["Y"]][, j]), .combine = c) %dopar%
        estimateStat(
          ...
          s = mod$lambda.min,
          statistic = rsq)
    RSQ[[nm]] <- c(RSQ[[nm]], stat)
  }
}
par(mar = c(14, 4, 4, 2) + 0.1)
rsq_bplot <-
  graphics::boxplot(RSQ,
        ...
        las=2,
        cex.axis = 0.8)
abline(h = median(RSQ[["remap_core_collapsed_rand"]]), col = "red", lty = 2)
```
on M 24h
...
models_cv <- list()
for (nm in xnames) {
  models_cv[[nm]] <- foreach(y = iterators::iter(dataM[["Y"]][, j])) %dopar%
    glmnet::cv.glmnet(
      x = dataM[[nm]],
      y = y[, 1],
      alpha = 0,
...
for (nm in xnames) {
  for(mod in models_cv[[nm]]) {
    stat <-
      foreach(y = iterators::iter(dataM[["Y"]][, j]), .combine = c) %dopar%
        estimateStat(
          x = dataM[[nm]],
          y = y[, 1],
          u = dataM[["U"]],
...
mse_bplot <-
  graphics::boxplot(MSE,
        main = "CV MSE across replicates, MDA-MB-231",
        ylab = "MSE",
        xlab = "",
        notch = TRUE,
        las=2,
...
i <- rownames(regression_elaborate_avg[["A_549"]][["remap_core_collapsed"]])[regpval < log(0.05)]
