...
makeTFHeatmap <- function(tf_mat, meta) {
  print(tf_mat %>% dim())
  
  # tf experiments correlation
  tf_cor <- cor(tf_mat %>% as.matrix())
  tf_cor_d <- as.dist(abs((1 - tf_cor) / 2))
  # tf_remap_binary_d <- dist(Matrix::t(tf_remap), method = "binary") # binary
  ...
  annotation_colors <- lapply(
    ...
    FUN = function(x) {
      ...
      m <- ! duplicated(levels(x)) & ! duplicated(levels(x), fromLast = TRUE)
      col[m] <- NA
    }
  )
...
tf_mat <- tf_mat[, Matrix::colSums(tf_mat) > 0]

coverageHist(Matrix::rowSums(tf_mat), main = paste0(sel_tf, " rowSums"))



makeTFHeatmap(tf_mat[Matrix::rowSums(tf_mat) > 0, ], remap_tfs)
```

```{r tp53_chip_atlas}
sel_tf <- "TP53"
sep <- "_"
chip_atlas_tfs[tf == sel_tf, 
          .(
            n_id = length(id),
            n_study = length(unique(study)),
            n_biotype = length(unique(biotype))
...
m <- grepl(paste0("^", sel_tf, sep), colnames(chip_atlas_promoters_core))
tf_chip_atlas <- chip_atlas_promoters_core[, m]

coverageHist(Matrix::colSums(tf_chip_atlas), main = paste0(sel_tf, " coverage"))

# remove 0 coverage experiments
tf_chip_atlas <- tf_chip_atlas[, Matrix::colSums(tf_chip_atlas) > 0]

# tf experiments correlation
tf_cor <- cor(tf_chip_atlas %>% as.matrix())
tf_cor_d <- as.dist(abs((1 - tf_cor) / 2))
tf_binary_d <- dist(Matrix::t(tf_chip_atlas), method = "binary") # binary
...
annotaion <- data.frame(
  ...
  biotype = tf_cor %>%
    rownames() %>%
    gsub(paste0(sel_tf, "_.*", sep, "(.*)", sep, ".*"), "\\1", .),
  row.names = tf_cor %>% rownames(),
  stringsAsFactors = TRUE
)
annotation_colors <- lapply(
  X = annotaion,
  FUN = function(x) stats::setNames(colorspace::qualitative_hcl(nlevels(x)), levels(x))
)
labs <- tf_cor %>% 
  rownames() %>% 
  gsub(paste0(sel_tf, sep, ".*", sep, "(.*)", sep, "(.*)"), "\\1 \\2", .)
    
pheatmap::pheatmap(mat = tf_cor, 
                   scale = "none", 
                   cluster_rows = TRUE,
                   cluster_cols = TRUE, 
                   clustering_distance_rows = tf_cor_d,
...
m <- grepl(paste0(sep, sel_tf, sep), colnames(remap_promoters_core))
tf_remap <- remap_promoters_core[, m]

coverageHist(Matrix::colSums(tf_remap), main = paste0(sel_tf, " coverage"))

# remove 0 coverage experiments
tf_remap <- tf_remap[, Matrix::colSums(tf_remap) > 0]

# tf experiments correlation
tf_remap_cor <- cor(tf_remap %>% as.matrix())
tf_remap_cor_d <- as.dist(abs((1 - tf_remap_cor) / 2))
tf_remap_binary_d <- dist(Matrix::t(tf_remap), method = "binary") # binary
...
annotaion <- data.frame(
  study = tf_remap_cor %>%
    rownames() %>%
    gsub(paste0(sep, sel_tf, ".*"), "", .),
  biotype = tf_remap_cor %>%
    rownames() %>%
    gsub(paste0(".*", sel_tf, sep), "", .) %>%
    gsub("_.*", "", .),
  row.names = tf_remap_cor %>% rownames(),
  stringsAsFactors = TRUE
)
annotation_colors <- lapply(
  X = annotaion,
  FUN = function(x) stats::setNames(pals::glasbey(nlevels(x)), levels(x))
)
labs <- tf_remap_cor %>%
                     rownames() %>% 
                     gsub(paste0(sep, sel_tf, sep), " ", .)
    
pheatmap::pheatmap(mat = tf_remap_cor, 
                   scale = "none", 
                   cluster_rows = TRUE,
                   cluster_cols = TRUE, 
                   clustering_distance_rows = tf_remap_cor_d,
                   clustering_distance_cols = tf_remap_cor_d,
                   fontsize = 3,
                   labels_col = labs,
                   labels_row = labs,
                   annotation_row = annotaion,
